  ...
  return (
    ...
    >
      ...
      <Host style={{ flex: 1 }} colorScheme={theme}>
        <VStack spacing={12} modifiers={[padding({ top: 16, horizontal: 16 })]}>
          <HStack>
            <Text modifiers={[font({ size: 22, weight: "bold" })]}>Add to watchlist</Text>
            <Spacer />
            <Button label="Close" onPress={close} modifiers={[foregroundStyle(String(primary))]} />
          </HStack>
          ...
          {status ? (
            <Text modifiers={[font({ size: 14 }), foregroundStyle(String(muted))]}>
              {status}
            </Text>
          ) : null}
